...
// `epsilon`, epsilon, is a stopping criterion when we want to iterate until we're "close enough".
mctad.epsilon = 0.0001;
;
// ## isInteger(n)
...
mctad.meanDirection = function (data) {
  ...
  if (C_bar >= 0 ) {
    ...
    θ_bar = Math.atan(S_bar/C_bar) + mctad.PI;
  }
  return θ_bar;
...
mctad.medianDirection = function (data) {
  ...
  // (i)  half of the data points lie in the in the arc [θ, θ + PI) and
  // (ii) the majority are nearer to θ than to θ + PI
  //
...
mctad.sampleVariance = function (data) {
  ...
  var mean = 0.0, sigma2 = 0.0, delta, n = 0, M2 = 0.0;
  for (var i = 0; i < data.length; i++) {
    n++;
    delta = data[i] - mean;
    mean += delta/n;
...
mctad.binomial = function (n, p) {
  ...
  while (dfs[x - 1].cdf < 1.0 - mctad.epsilon);
  dfs.domain.max = x - 1;
...
mctad.pascal = function (r, p) {
  ...
  while (dfs[k - 1].cdf < 1.0 - mctad.epsilon);
  dfs.domain.max = k - 1;
...
mctad.chiSquared = function (k) {
  ...
  var dfs = {
    ...
    pdf: function (x) {
      if (x > 0) {
        return (1 / (Math.pow(2, k / 2) * mctad.gamma(k / 2))) * Math.pow(x, (k / 2) - 1) * Math.pow(Math.E, -x / 2);
      } else {
        return 0.0;
      }
    },
...
mctad.inverseErf = function (x) {
  ...
  return mctad.sign(x) * Math.sqrt(
    Math.sqrt(
      Math.pow((2 / (mctad.PI * a) + (Math.log(1 - Math.pow(x, 2)) / 2)), 2) - Math.log(1 - Math.pow(x, 2)) / a
    ) - ((2 / (mctad.PI * a)) + Math.log(1 - Math.pow(x, 2)) / 2)
  );
};
;
mctad.lognormal = function (μ, sigma2) {
  // Check that `μ > 0` and `sigma2 > 0`.
  if (μ < 0 || sigma2 <= 0) { return undefined; }

  var dfs = {
    mean: Math.pow(Math.E, μ + sigma2 / 2),
    median: Math.pow(Math.E, μ),
    mode: Math.pow(Math.E, μ - sigma2),
    variance: Math.pow(Math.E, 2 * μ + sigma2) * (Math.pow(Math.E, sigma2) - 1),
    skewness: (Math.pow(Math.E, sigma2) + 2) * Math.sqrt(Math.pow(Math.E, sigma2) - 1),
    entropy: 0.5 + 0.5 * Math.log(2 * mctad.PI * sigma2) + μ,
    ...
    generate: function (n) {
      var randomVariables = [];
      randomVariables = mctad.normal(μ, sigma2).generate(n);
      for (var i = 0; i < n; i++ ) {
    ...
    pdf: function (x) {
      if (x > 0) {
        return (1 / (x * Math.sqrt(2 * mctad.PI * sigma2))) * Math.pow(Math.E, -(Math.pow(Math.log(x) - μ, 2) / (2 * sigma2)));
      } else {
        return undefined;
      }
    },
    ...
    cdf: function (x) {
      if (x > 0) {
        var Z = (Math.log(x) - μ) / Math.sqrt(sigma2);
...
;
mctad.normal = function (μ, sigma2) {
  // Check that `sigma2 > 0`.
  if (sigma2 <= 0) { return undefined; }
  ...
  var dfs = {
    ...
    variance: sigma2,
    skewness: 0,
    entropy: 0.5 * Math.log(2 * mctad.PI * Math.E * sigma2),
    ...
    generate: function (n) {
      ...
      randomVariables.push(μ + Math.sqrt(sigma2) * (V[0] * Y), μ + Math.sqrt(sigma2) * (V[1] * Y));
      }
    ...
    pdf: function (x) {
      return (1 / (Math.sqrt(sigma2) * Math.sqrt(2 * mctad.PI))) * Math.pow(Math.E, -(Math.pow(x - μ, 2) / (2 * sigma2)));
    },

    cdf: function (x) {
      var Z = (x - μ) / Math.sqrt(2 * sigma2);
...
mctad.studentsT = function (v) {
  ...
  var dfs = {
    ...
    pdf: function (x) {
      return (mctad.gamma((v + 1) / 2) / (Math.sqrt(v * mctad.PI) * mctad.gamma(v / 2)) * Math.pow((1 + Math.pow(x, 2) / v), -((v + 1) / 2)));
    },
...
mctad.weibull = function (λ, k) {
  ...
  var dfs = {
    ...
    variance: Math.pow(λ, 2) * ( mctad.gamma(1 + 2 / k) - Math.pow(mctad.gamma(1 + 1 / k), 2)),
...
mctad.confidenceIntervalOnTheDifferenceBetweenTwoMeans = function (x_bar, s_x, y_bar, s_y, n_x, n_y, α, type) {
  ...
  var
    ...
    sigma_x_bar_minus_y_bar = Math.sqrt(Math.pow(s_x, 2) / n_x + Math.pow(s_y, 2) / n_y),
    v;
  ...
  if (n_x > 30 && n_y > 30) {
    ...
    if (typeof type !== 'undefined' && type.toLowerCase() === 'u') {
      return x_bar_minus_y_bar + mctad.z(1 - α) * sigma_x_bar_minus_y_bar;
    } else {
      ...
      if (typeof type !== 'undefined' && type.toLowerCase() === 'l') {
        ...
        return [
          x_bar_minus_y_bar - mctad.z(1 - α / 2) * sigma_x_bar_minus_y_bar,
          x_bar_minus_y_bar + mctad.z(1 - α / 2) * sigma_x_bar_minus_y_bar
        ];
      }
    }
    ...
    // Return the upper confidence bound of a one-tailed confidence interval.
    if (typeof type !== 'undefined' && type.toLowerCase() === 'u') {
      return x_bar_minus_y_bar + mctad.t_distribution_table[v][1 - α] * sigma_x_bar_minus_y_bar;
    } else {
      // Return the lower confidence bound of a one-tailed confidence interval.
      if (typeof type !== 'undefined' && type.toLowerCase() === 'l') {
        return x_bar_minus_y_bar - mctad.t_distribution_table[v][1 - α] * sigma_x_bar_minus_y_bar;
        ...
        // Return both bounds of a two-tailed confidence interval.
        return [
          x_bar_minus_y_bar - mctad.t_distribution_table[v][1 - α / 2] * sigma_x_bar_minus_y_bar,
          x_bar_minus_y_bar + mctad.t_distribution_table[v][1 - α / 2] * sigma_x_bar_minus_y_bar
        ];
      }
    }
...
mctad.confidenceIntervalOnTheDifferenceBetweenTwoProportions = function (X, Y, n_x, n_y, α, type) {
  ...
  var
    ...
    sigma_p_tilde_x_p_tilde_y = Math.sqrt(p_tilde_x * (1.0 - p_tilde_x) / n_tilde_x + p_tilde_y * (1.0 - p_tilde_y) / n_tilde_y);

  // Return the upper confidence bound of a one-tailed confidence interval.
  if (typeof type !== 'undefined' && type.toLowerCase() === 'u') {
    return p_tilde_x_minus_p_tilde_y + mctad.z(1 - α) * sigma_p_tilde_x_p_tilde_y;
  } else {
    // Return the lower confidence bound of a one-tailed confidence interval.
    if (typeof type !== 'undefined' && type.toLowerCase() === 'l') {
      return p_tilde_x_minus_p_tilde_y - mctad.z(1 - α) * sigma_p_tilde_x_p_tilde_y;
      ...
      // Return both bounds of a two-tailed confidence interval.
      return [
        p_tilde_x_minus_p_tilde_y - mctad.z(1 - α / 2) * sigma_p_tilde_x_p_tilde_y,
        p_tilde_x_minus_p_tilde_y + mctad.z(1 - α / 2) * sigma_p_tilde_x_p_tilde_y
      ];
    }
  }
...
mctad.confidenceIntervalOnTheMean = function (x_bar, s, n, α, type) {
  ...
  var sigma_bar = s / Math.sqrt(n);

  // If the sample size is large, use Z Scores from the Standard Normal Distribution.
  if (n > 30) {
    // Return the upper confidence bound of a one-tailed confidence interval.
    if (typeof type !== 'undefined' && type.toLowerCase() === 'u') {
      return x_bar + mctad.z(1 - α) * sigma_bar;
    } else {
      // Return the lower confidence bound of a one-tailed confidence interval.
      if (typeof type !== 'undefined' && type.toLowerCase() === 'l') {
        return x_bar - mctad.z(1 - α) * sigma_bar;
    ...
    // Return the upper confidence bound of a one-tailed confidence interval.
    if (typeof type !== 'undefined' && type.toLowerCase() === 'u') {
      return x_bar + mctad.t_distribution_table[n - 1][1 - α] * sigma_bar;
    } else {
      // Return the lower confidence bound of a one-tailed confidence interval.
      if (typeof type !== 'undefined' && type.toLowerCase() === 'l') {
        return x_bar - mctad.t_distribution_table[n - 1][1 - α] * sigma_bar;
        ...
        // Return both bounds of a two-tailed confidence interval.
        return [
          x_bar - mctad.t_distribution_table[n - 1][1 - α / 2] * sigma_bar,
          x_bar + mctad.t_distribution_table[n - 1][1 - α / 2] * sigma_bar
        ];
      }
    }
...
mctad.confidenceIntervalOnTheProportion = function (X, n, α, type) {
  ...
  var
    n_tilde = n + 4,
    p_tilde = (X + 2) / n_tilde,
    sigma_p_tilde = Math.sqrt(p_tilde * (1.0 - p_tilde) / n_tilde);

  // Return the upper confidence bound of a one-tailed confidence interval.
  if (typeof type !== 'undefined' && type.toLowerCase() === 'u') {
    return p_tilde + mctad.z(1 - α) * sigma_p_tilde;
  } else {
    // Return the lower confidence bound of a one-tailed confidence interval; enforce 0 as lowest possible bound.
    if (typeof type !== 'undefined' && type.toLowerCase() === 'l') {
      return Math.max(0, p_tilde - mctad.z(1 - α) * sigma_p_tilde);
    } else {
      // Return both bounds of a two-tailed confidence interval; enforce 0 as lowest possible bound.
      return [
        Math.max(0, p_tilde - mctad.z(1 - α / 2) * sigma_p_tilde),
        p_tilde + mctad.z(1 - α / 2) * sigma_p_tilde
      ];
    }
  }
